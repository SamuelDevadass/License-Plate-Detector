  ...
  return (
    <div className="panel">
      ...
      {loading ? ( <p>Loading wings…</p>) : 
        wings.length === 0 ? ( <div className="error-banner">
          No wings found. Check the backend connection.</div> ) : 
        (
          <div className="field">
            <label htmlFor="wing-select">Wing</label>
            <select id="wing-select" value={data.wing} onChange={handleWingChange}>
              <option value="" disabled>
                Choose a wing ... </option>
                {wings.map((w) => ( <option key={w} value={w}> {w}
              </option>))}
            </select>
          </div>)}
      <div className="btn-row">
        <button className="btn btn--primary" disabled={!data.wing} onClick={() => goTo("spots")}>
          Continue </button>
      </div>
    </div>
  );
